...
5. 효율성을 따져서 데이터가 어느 쪽으로 더 자주 사용될 것인가 예측
   ...
   ```
5. N+1 문제
   - 위에서 발생한 쿼리처럼 하나의 쿼리를 발생시켰지만 4개의 쿼리가 발생한다.
     - 이벤트 조회 -> 잘못된 쿼리 최적화 과정에서 이벤트들이 바라보는 장소들이 각각 SELECT 발생
   - 이것이 N+1문제
6. N+1 문제를 해결하는 3가지 방법
   1. 똑똑한 Lazy
      - 비즈니스 로직을 면밀히 분석해서 불필요한 연관 관계 테이블 정보를 불러오는 부분을 제거
      - 가장 효율적인 방법으로 가장 OOP적인 설계
      - 비즈니스 로직에서 연관 테이블의 정보를 피하는게 어려울 경우 불가능
   2. eager fetch + join jpql
      - join 쿼리를 직접 작성하는 여러가지 방법 (@Query, querydsl 등등)
      - 쿼리를 한번에 가져오지만, join 연산 비용과 네트워크로 전달되는 데이터가 큰 문제 
   3. 후속 쿼리를 in으로 묶어주기
      - 모든 쿼리에 적용되고, 복잡한 도메인인에서 join 쿼리 구성이 어려울 때 효율적일 수 있다.
      - N+1 -> 1+1로 I/O를 줄인다.
      - 하이버네이트의 프로퍼티인 dafault_batch_fetch_size
      - 100~1000사이를 자주 사용하며
7. 해결하는 방법 구현
   1. 똑똑한 Lazy
      - 연관 관계의 필드를 지운다.
      - 하지만 비즈니스 로직상에서 꼭 필요할 땐 사용 불가능
   2. eager fetch + join jpql
      - QueryDSL로 직접 구현한 쿼리로 조인을 수행하도록 한다.
   3. 후속 쿼리를 in으로 묶어주기
